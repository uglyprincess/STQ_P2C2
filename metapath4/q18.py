...
import numpy as np
from collections import defaultdict## D ---> PR ---> PR Review ---> Developer
...
class smart_shark:
    ...
    def __init__(self):
        ...
        # self.less_rejected_pr_ranking = dict()
    ...
    def find_more_than_mean_prs(self):
                ...
                if(corresponding_pr == pr["_id"] and review["state"] != decision):
                    ...
                    if(review_time - creation_time > self.reject_pr_mean_time):
                        ...
                        for file in list_files_in_pr:
                            ...
                            # self.more_rejected_pr_ranking[developer_id] = review_time - creation_time
                            self.more_rejected_pr_ranking.append(
                                {
        ...
        # for doc in self.more_rejected_pr_ranking:
        #     print(doc)
        
        return self.more_rejected_pr_ranking
            ...
            for pr in self.list_prs:
                ...
                if(corresponding_pr == pr["_id"] and review["state"] != decision):
                    ...
                    if(review_time - creation_time > self.reject_pr_mean_time):
                        ...
                        if(reviewer_id not in self.rejected_pr_ranking):
                            self.rejected_pr_ranking[reviewer_id] = 1
                        else:
                            self.rejected_pr_ranking[reviewer_id] = self.rejected_pr_ranking[reviewer_id] + 1
                                                
        for rejected_prs in dict(sorted(self.rejected_pr_ranking.items(), key=lambda item: item[1])):
            print(f"Reviewer {rejected_prs} reviewed {self.rejected_pr_ranking[rejected_prs]} PRs in more than the mean time")
